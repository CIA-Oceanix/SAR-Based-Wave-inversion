...
class Sar_to_Sar(pl.LightningModule):
    def __init__(self):
        super().__init__()
...
class build_unet(pl.LightningModule):
    ...
    def loss(self,pred_spec,true_spec,sim_spec):     
        l1 = 1000*F.mse_loss(pred_spec[0].squeeze(),true_spec)
        l2 = 2*1000*F.mse_loss(pred_spec[1].squeeze(),sim_spec)
        return l1+l2
    ...
    def training_step(self,batch,batch_idx):
        ...
        for i in range(l):
            ...
            sim[i][0] = sim[i][0]/torch.max(sim[i][0])
            sim[i][1] = sim[i][1]/torch.max(sim[i][1])
        
        pred_spec=self(sar.to(torch.float))
        loss= self.loss(pred_spec,ww3.to(torch.float),sim.to(torch.float))
        self.log('loss',loss)
        return {'loss' : loss}
...
logger = pl.loggers.TensorBoardLogger(name=f'MPP4+MI4-simu-lr={config["lr"]}', save_dir='inversion_logs')
